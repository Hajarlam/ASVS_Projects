...
export class AiService {
  ...
  private readonly apiKeys: string[] = [
    'AIzaSyCZz6paDPqNro-EppiCDQQHS1QB7ZEtZW4', // clé 1 — remplacer si épuisée
  ...
  private async generateWithFallback(
    ...
    for (let keyAttempt = 0; keyAttempt < this.apiKeys.length; keyAttempt++) {
      ...
      if (!this.keyQuotaExhausted.has(keyIndex)) break;
    }

    throw lastError || new Error('Quota Gemini atteint sur toutes les clés. Ajoutez des clés dans ai.service.ts');
  }
  ...
  private async ensureCompleteWithCode(draft: string, context: string, skipContinuation = false): Promise<string> {
...
Reponse actuelle (CONTINUE EXACTEMENT ICI, dernier mot inclus):
${output.slice(-2600)}

Regles ABSOLUES:
- Continue a partir du dernier mot exact, sans le repeter.
- Termine toutes les phrases et sections inachevees.
- Ferme les blocs markdown de code ouverts.
- Pas de salutation, pas d'introduction.`,
    ...
    if (!this.hasCodeBlock(output)) {
      const looksLikeScanReport = /score\s+(sécurité|securite)|vuln-\d+|owasp\s+top\s+10|actions\s+immédiates/i.test(output);
      if (!looksLikeScanReport) {
  ...
  private async executeMcpToolBackendLegacy(tool: string, args: Record<string, any>): Promise<string | null> {
    ...
    const call = async (path: string, body: Record<string, any>): Promise<string | null> => {
      const res = await this.fetchWithTimeout(`${BACKEND_URL}${path}`, {
        method: 'POST',
        headers,
  ...
  async getExplanation(requirement: AiRequest): Promise<string> {
    if (this.auth.isBackendMode() && this.isBrowser) {
      try {
        const res = await this.fetchWithTimeout(`${BACKEND_URL}/api/mcp/analyze`, {
          method: 'POST',
...
## Checklist de Verification`,
  ...
  async chat(message: string, history: { role: string; text: string }[], options: ChatOptions = {}): Promise<string> {
    ...
    if (this.auth.isBackendMode() && this.isBrowser) {
      try {
        const res = await this.fetchWithTimeout(`${BACKEND_URL}/api/mcp/chat`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${this.auth.getToken()}` },
        ...
        if (res.ok) {
          ...
          if (this.isUsableResult(d?.result)) {
            return await this.ensureCompleteWithCode(d.result, message, true);
          }
        }
      } catch { }
    }
...
User: ${message}`,
      ...
      return await this.ensureCompleteWithCode(base, message, fastMode);
    } catch (e: any) {
      if (this.isQuotaError(e)) {
        return this.buildLocalChatFallback(message, fastMode);
      }
  ...
  async scanCode(code: string, language: string): Promise<string> {
    if (this.auth.isBackendMode() && this.isBrowser) {
      try {
        const res = await this.fetchWithTimeout(`${BACKEND_URL}/api/mcp/scan-code`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${this.auth.getToken()}` },
        ...
        if (res.ok) {
          const d = await res.json();
          if (this.isUsableResult(d?.result)) {
            return await this.ensureCompleteWithCode(d.result, `scan code ${language}`, true);
          }
        }
      } catch { }
    }
...
## Top 3 recommandations`
  ...
  async getMcpStatus(): Promise<any> {
    ...
    try {
      const res = await this.fetchWithTimeout(`${BACKEND_URL}/api/mcp/status`, {
  ...
  async getMcpTools(): Promise<McpToolDescriptor[]> {
    ...
    try {
      const res = await this.fetchWithTimeout(`${BACKEND_URL}/api/mcp/tools`, {
        headers: { 'Authorization': `Bearer ${this.auth.getToken()}` }
      });
      if (!res.ok) return [];
  ...
  private async scanRepositoryWithGemini(repoUrl: string): Promise<string> {
    ...
    try {
      const r = await this.generateWithFallback(model => ({
        model,
        contents: `Tu es un expert en cybersécurité spécialisé OWASP ASVS. Effectue une analyse de sécurité approfondie du repository GitHub suivant:

**Repository:** ${owner}/${repoName}
**URL:** ${repoUrl}

Analyse le nom du repo, sa structure probable et son type de projet pour fournir une analyse SPÉCIFIQUE et RÉALISTE. Ne donne PAS une réponse générique.

Réponds UNIQUEMENT en français, format markdown strict, sans introduction:

## 🛡️ Score Sécurité: [X]/100
> [Justification courte du score en 1 phrase]

## 🔴 Vulnérabilités Critiques & Hautes

**VULN-01** | Critique | OWASP A0X: [nom]
[Description précise et contextualisée pour ce type de projet]
\`\`\`[langage]
// ❌ Problème typique
[code problématique]
...
1. **[action]** — [détail concret]
2. **[action]** — [détail concret]
3. **[action]** — [détail concret]
4. **[action]** — [détail concret]
5. **[action]** — [détail concret]

## 📊 OWASP Top 10
| # | Risque | Statut | Impact |
|---|--------|--------|--------|
| A01 | Broken Access Control | ✅/⚠️/🔴 | [court] |
| A02 | Cryptographic Failures | ✅/⚠️/🔴 | [court] |
| A03 | Injection | ✅/⚠️/🔴 | [court] |
| A05 | Security Misconfiguration | ✅/⚠️/🔴 | [court] |
| A06 | Vulnerable Components | ✅/⚠️/🔴 | [court] |
| A07 | Auth Failures | ✅/⚠️/🔴 | [court] |`,
      ...
      if (this.isQuotaError(e)) {
        return `## ⏳ Quota Gemini temporairement atteint\n\nRéessayez dans 1-2 minutes. Le service sera automatiquement rétabli.\n\n*Repository analysé: ${repoUrl}*`;
      }
  ...
  async executeMcpTool(tool: string, args: Record<string, any>): Promise<string> {
    ...
    if (tool === 'scan_repository') {
      ...
      const isFallbackResponse = (text: string): boolean => {
        ...
        return t.includes('fallback sans ia')
          || t.includes('gemini est temporairement indisponible')
          || t.includes('rapport de secours')
          || t.includes('quota/rate limit')
          || t.includes('verification manuelle prioritaire')
          || t.includes('recherche de concatenation sql')
      ...
      try {
        const res = await this.fetchWithTimeout(`${BACKEND_URL}/api/mcp/public/scan-repository`, {
          ...
          body: JSON.stringify({ repoUrl, branch: String(args?.['branch'] || '') })
        }, 25000);
    ...
    try {
      const res = await this.fetchWithTimeout(`${BACKEND_URL}/api/mcp/execute`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${this.auth.getToken()}`
